...
export default class IPay extends Component {
  ...
  componentDidMount() {
    if (isAndroid) {
      ...
      failedSubscription = DeviceEventEmitter.addListener(
        "ipay88:failed",
        (data) => this.onFailed(data)
      );
      cancelSubscription = DeviceEventEmitter.addListener(
        "ipay88:canceled",
        (data) => this.onCanceled(data)
      );
    } else {
      // ios
      successSubscription = iosEvent.addListener("ipay88:success", (data) =>
        this.onSuccess(data)
      );
      failedSubscription = iosEvent.addListener("ipay88:failed", (data) =>
        this.onFailed(data)
      );
      cancelSubscription = iosEvent.addListener("ipay88:canceled", (data) =>
